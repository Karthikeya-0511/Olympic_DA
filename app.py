...
if user_menu == 'Medal Tally':
    ...
    st.dataframe(medal_tally, height=300)

if user_menu == 'OverAll Analysis':
    editons = df['Year'].unique().shape[0] - 1
    ...
    fig = px.line(nation_over_time, x='Edition',
                  y='region')
    st.title('Participating Nations Over the Years')
    st.plotly_chart(fig)

    events_over_time = helper.data_over_time(df,'Event')
    fig = px.line(events_over_time, x='Edition',
                  y='Event')
    st.title('Events Over the Years')
    st.plotly_chart(fig)

    athlete_over_time = helper.data_over_time(df, 'Name')
    fig = px.line(athlete_over_time, x='Edition',
                  y='Name')
